<template>
  <div>
  <el-collapse v-model="activeName" accordion v-for="(diary,index) in diarys" :key="diarys.length">
    <el-collapse-item title="努力寻找中："  >
      ...
      <div>奖赏：<span>{{diary.reward}}</span>
        <div style="position: absolute;right: 2px"><span>{{diary.lpTime}}</span></div></div>
    </el-collapse-item>
  </el-collapse>
    </div>
</template>
